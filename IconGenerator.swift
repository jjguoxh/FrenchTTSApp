...
func saveIcon(size: Int, filename: String) {
    let view = AppIconView().frame(width: CGFloat(size), height: CGFloat(size))
    let hostingView = NSHostingView(rootView: view)
    hostingView.frame = CGRect(x: 0, y: 0, width: CGFloat(size), height: CGFloat(size))
    ...
    if let data = bitmapRep.representation(using: .png, properties: [:]) {
        let url = URL(fileURLWithPath: "Assets.xcassets/AppIcon.appiconset/\(filename)")
...
for size in sizes {
    saveIcon(size: size, filename: "AppIcon-\(size).png")
}
